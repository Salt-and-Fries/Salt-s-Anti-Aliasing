 ...
 *
 * Temporal resolve pass that blends current and historical samples while clamping history to the current neighborhood.
...
uniform float BaseHistoryWeight;
uniform float LumaRejection;
uniform float DepthRejection;
uniform float NeighborhoodClamp;
uniform float CurrentJitterX;
uniform float CurrentJitterY;
uniform float PreviousJitterX;
uniform float PreviousJitterY;
...
vec3 sampleCurrent(vec2 offset) {
    return texture(DiffuseSampler, texCoord + offset).rgb;
}
