...
uniform float VarianceGuard;
uniform float EdgeConfidenceScale;
...
void main() {
    ...
    vec3 rgbNW = texture(DiffuseSampler, texCoord + vec2(-texel.x, -texel.y)).rgb;
    vec3 rgbNE = texture(DiffuseSampler, texCoord + vec2(texel.x, -texel.y)).rgb;
    vec3 rgbSW = texture(DiffuseSampler, texCoord + vec2(-texel.x, texel.y)).rgb;
    vec3 rgbSE = texture(DiffuseSampler, texCoord + vec2(texel.x, texel.y)).rgb;
