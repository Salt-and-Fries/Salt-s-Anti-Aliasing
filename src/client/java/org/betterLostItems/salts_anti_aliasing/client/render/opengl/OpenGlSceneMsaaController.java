...
/**
 * Manages OpenGL multisample scene targets for the 1.21.1 framebuffer renderer.
 */
public final class OpenGlSceneMsaaController {
    ...
    public boolean overrideMainFramebuffer(RenderTarget target, boolean setViewport) {
        if (!active || target != mainTarget || msaaFramebufferId == 0) {
            return false;
        }

        RenderSystem.assertOnRenderThread();
        GlStateManager._glBindFramebuffer(GL30C.GL_FRAMEBUFFER, msaaFramebufferId);
        if (setViewport) {
    ...
    private void bindMsaa(RenderTarget target) {
        GlStateManager._glBindFramebuffer(GL30C.GL_FRAMEBUFFER, msaaFramebufferId);
        GlStateManager._viewport(0, 0, target.viewWidth, target.viewHeight);
        mainColorDirty = true;
        mainDepthDirty = true;
    ...
    private void syncMainTargetIfNeeded(boolean includeColor, boolean includeDepth) {
        if (!active || mainTarget == null || msaaFramebufferId == 0) {
            return;
        }
        ...
        GL30C.glBindFramebuffer(GL30C.GL_DRAW_FRAMEBUFFER, mainTarget.frameBufferId);
